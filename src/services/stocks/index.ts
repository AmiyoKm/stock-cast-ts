...
export const getStockHistoryByID = async (tradingCode: string, start: Date, end: Date): Promise<Stock[]> => {
    const query = `SELECT id, date, trading_code, ltp, high, low, openp, closep, ycp, trade, value, volume
              FROM stock_history
...
export const getStockByID = async (tradingCode: string): Promise<Stock> => {
    const query = `
